...
class Widget_TMEA_bbpress_Search extends Widget_Base {

	public function get_name() {
		return 'tme-bbpress_search';
	}

	public function get_title() {
		return "[TM] " . esc_html__( 'bbPress Search', 'theme-masters-elementor' );
	}
	...
	public function get_script_depends() {
		return [ 'jquery-ui-autocomplete', 'tme-bbpress_search' ];
    }
	...
	protected function register_controls() {
		...
		$this->add_control(
			'placeholder', [
				...
				'default' => esc_html__( 'Search Forums...', 'theme-masters-elementor' ),
				'label_block' => true,
			]
		);
    ...
    protected function render() {
        ...
        <div class="tmea-search-form-container <?php if (is_rtl()) { echo 'form-is-rtl'; } ?>" data-smax="<?php echo esc_attr($settings['max']); ?>" data-smin="<?php echo esc_attr($settings['min']); ?>"  data-sdelay="<?php echo esc_attr($settings['delay']); ?>">
            <form id="tmea-search-form-<?php echo $widget_id; ?>" role="search" method="get" class="tmea-search-form ui-front" style="display:flex;" action="<?php bbp_search_url(); ?>">
                <input tabindex="<?php bbp_tab_index(); ?>" type="text" class="tmea-search-form-term autocomplete-<?php echo $settings['autocomplete']; ?><?php if (is_rtl()) { echo '-rtl'; } ?>" placeholder="<?php echo esc_attr($settings['placeholder']); ?>" name="bbp_search" value="<?php echo esc_attr( bbp_get_search_terms() ); ?>" />
		...
		if ($settings['autocomplete']) {
			wp_localize_script( 'tme-bbpress_search', 'tmeabbpressAutocomplete', array( 'url' => admin_url( 'admin-ajax.php' ) ) ); 
		}
		?>
	<?php }
}
Plugin::instance()->widgets_manager->register_widget_type( new Widget_TMEA_bbpress_Search() );