...
namespace Crowdfundly\App\Helpers\Elementor;
...
/**
 * Elementor Single Campaign Widget
 * 
 * @package     crowdfundly
 * @author      Keramot UL Islam <sourav926>
 * @since       1.0.0
 */
class SingleCampaignWidget extends Widget_Base
{
	...
	protected function register_controls()
	{
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} #updates .campaign-view-updates__single-text-line p, {{WRAPPER}} #updates .campaign-view-updates__single-text-line p > span',
				'scheme' => Typography::TYPOGRAPHY_4,
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} #updates .campaign-view-updates__single-text-line p' => 'color: {{VALUE}} !important;',
					'{{WRAPPER}} #updates .campaign-view-updates__single-text-line p > span' => 'color: {{VALUE}} !important;'
				],
			]
		);
	...
	{
		// $campaign = \Crowdfundly_Helper::get_all_campaigns();
		$campaigns = AllCampaignController::get_all_campaigns();
		
		$camp_list = [ '' => 'Select Campaign' ];
		if ( ! $campaigns || empty( $campaigns['data'] ) ) return $camp_list;
